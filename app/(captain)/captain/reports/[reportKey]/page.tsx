...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: def ? `${def.title} — Captain reports` : 'Report — Captain',
  };
}
...
export default async function CaptainReportDetailPage({ params, searchParams }: PageProps) {
  const session = await getServerSession();
  if (!session) {
    const { reportKey } = await params;
    redirect(`/login?next=/captain/reports/${reportKey}`);
  }
  ...
  return (
    <main className="p-4 sm:p-6 lg:p-8 max-w-[1600px] mx-auto space-y-5">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        ...
        <Link
          href={`/captain/reports/${reportKey}/export?${exportParams.toString()}`}
      ...
      <ReportFiltersBar
        ...
        cityId={cityId ?? 'all'}
        search={search}
        basePath={`/captain/reports/${reportKey}`}
      />

      {def.defaultBucket && (
        <ReportBucketToggle active={bucket} basePath={`/captain/reports/${reportKey}`} />
      )}
      ...
      {totalPages > 1 && (
        <Pagination
          pathname={`/captain/reports/${reportKey}`}
          page={page}
