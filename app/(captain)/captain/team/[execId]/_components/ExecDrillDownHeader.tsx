  ...
  return (
    <>
      ...
      <section
        ...
        className="mx-auto max-w-2xl px-4 sm:px-6 pt-4 space-y-3"
      >
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1 space-y-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {exec.hasRedFlag && (
                <Badge variant="destructive" className="text-[10px]">
                  ⚑ {quickStats.overdueTaskCount} overdue
                </Badge>
              )}
            </div>
            {cities.length > 0 && (
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <span className="font-mono">{exec.phone}</span>
              <a
                href={`tel:${exec.phone}`}
            ...
            <p className="text-[11px] text-muted-foreground/80">
              Cities reflect the captain&apos;s assignments; per-exec city
              attribution is not modelled today.
            </p>
          </div>
        ...
        <dl className="grid grid-cols-4 gap-2 text-xs">
          <Stat label="Visits today" value={String(quickStats.visitsToday)} />
          <Stat
  ...
  return (
    <div className="rounded-xl border bg-card p-2 space-y-0.5">
      ...
      <dd
        className={
          destructive
            ? 'text-base font-semibold tracking-tight text-destructive'
