...
export interface CaptainNavItem {
  ...
  badgeCount?: number;
  /**
   * HVA-331: this item has no mirror under /admin/portal/[captainId]/.
   *
   * The admin-viewing-a-captain nav is generated by rewriting each href in
   * this list from `/captain/x` to `/admin/portal/<id>/x`. That rewrite
   * assumed every captain page had an impersonation twin. Three never did —
   * Dispatch (added by HVA-308 and never mirrored), Tasks and Tickets — so
   * the nav advertised three links that landed on the 404 page.
   *
   * Flagged here rather than filtered in the layout so the fact lives beside
   * the route it describes: whoever adds `/admin/portal/[captainId]/dispatch`
   * deletes the flag on the same line they are already looking at.
   */
  noAdminPortalRoute?: boolean;
}
...
export const CAPTAIN_NAV_ITEMS: CaptainNavItem[] = [
  ...
  { href: '/captain/dispatch', label: 'Dispatch', icon: 'local_shipping', noAdminPortalRoute: true },
  { href: '/captain/team', label: 'My Team', icon: 'groups' },
  // 2026-06-05: team-wide task list with sort/filter/pagination.
  { href: '/captain/tasks', label: 'Tasks', icon: 'task', noAdminPortalRoute: true },
